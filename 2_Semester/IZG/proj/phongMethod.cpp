...
const glm::vec3 color_green = glm::vec3(0, 0.5, 0);
const glm::vec3 color_yellow = glm::vec3(1, 1, 0);
const glm::vec3 color_white = glm::vec3(1, 1, 1);
...
void phong_VS(OutVertex&outVertex,InVertex const&inVertex,Uniforms const&uniforms){
  ...
  /// \image html images/vertex_shader_tasks.svg "Vizualizace vstupů a výstupů vertex shaderu" width=1000
	...
	glm::vec4 pos = glm::vec4(inVertex.attributes[0].v3.x, inVertex.attributes[0].v3.y, inVertex.attributes[0].v3.z, 1);

	glm::mat4 viewMatrix = uniforms.uniform[0].m4;
	glm::mat4 projectionMatrix = uniforms.uniform[1].m4;
...
glm::vec3 phong_color(glm::vec3 pos)
{
	glm::vec3 color = color_yellow;
	float x = pos.x;
	float y = pos.y;
...
glm::vec3 normalize3(glm::vec3 vekt)
{
	float size = sqrt(vekt.x*vekt.x + vekt.y*vekt.y + vekt.z*vekt.z);
	for (int i = 0; i < 3; i++) vekt[i] /= size;
	return vekt;
}

float dot3(glm::vec3 a, glm::vec3 b)
{
...
{
	glm::vec3 out = vector - (2 * dot3(vector, normal)) * normal;
	return out;
}
...
void phong_FS(OutFragment&outFragment,InFragment const&inFragment,Uniforms const&uniforms){
  ...
  /// \image html images/fragment_shader_tasks.svg "Vizualizace výpočtu ve fragment shaderu" width=1000
	...
	glm::vec3 finColor = phong_color(inFragment.attributes[0].v3);

	glm::vec3 norm = normalize3(inFragment.attributes[1].v3);


	float t = 0;
	if (inFragment.attributes[1].v3.y > 0) t = norm.y * norm.y;

	glm::vec3 snowColor = color_white;

	for(int i = 0; i < 3; i ++) finColor[i] = finColor[i] * (1 - t) + t * snowColor[i];

	//hlavni zdroj pro phonga: https://www.fabrizioduroni.it/2017/07/26/phong-lighting-model.html
	
	float shine;
	shine = 40.f;
	
	glm::vec3 lightDirection = normalize3(uniforms.uniform[2].v3 - inFragment.attributes[0].v3);


	
	float diff = dot3(norm, lightDirection);
	if (diff < 0.0f) diff = 0.0f;
	glm::vec3 lightColor = color_white;

	glm::vec3 diffuse = lightColor * diff;

	glm::vec3 viewDirection = normalize3(uniforms.uniform[3].v3 - inFragment.attributes[0].v3);
	glm::vec3 reflectDirection = reflect(- lightDirection, norm);
	float spec = pow(getMax(dot3(viewDirection, reflectDirection), 0.0), shine);
	glm::vec3 specular = lightColor * spec;


	finColor = diffuse * finColor + specular;
	for (int i = 0; i < 3; i++) finColor[i] = finColor[i] > 1.0f ? 1.0f : finColor[i]; //sice to orezavam i ve funkci putPixel, ale testy ocekavaji ze oriznuti probehne zde

	outFragment.gl_FragColor = glm::vec4(finColor, 1.0f);
...
///  - gpu.setVS2FSType()
	...
	bunnyBuff = gpu.createBuffer(sizeof(bunnyVertices));
	indexBuff = gpu.createBuffer(sizeof(bunnyIndices));

	gpu.setBufferData(bunnyBuff, 0, sizeof(bunnyVertices), bunnyVertices);
	...
	gpu.setVertexPullerHead(vao, 0, AttributeType::VEC3, sizeof(BunnyVertex), 0, bunnyBuff);
	gpu.setVertexPullerHead(vao, 1, AttributeType::VEC3, sizeof(BunnyVertex), sizeof(BunnyVertex)/2, bunnyBuff);
...
PhongMethod::~PhongMethod(){
  ...
  ///  - gpu.deleteBuffer()
	...
	gpu.deleteBuffer(bunnyBuff);
	gpu.deleteBuffer(indexBuff);
