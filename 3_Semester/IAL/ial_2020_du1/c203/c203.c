...
*/
	...
	if(q == NULL) //osetreni zda je ukazatel na frontu validni
	{
		queueError(QERR_INIT);
		return;
	}

	for(int i = 0; i < QUEUE_SIZE; i++) //zaplnime pole
	{
		q->arr[i] = '*';
	}

	q->f_index = 0; //nyni se b==f, takze fronta je prazdna
	q->b_index = 0;
}
...
*/
	...
	return (index + 1) % QUEUE_SIZE; //timto pole vicemene zacyklime dokola, tim kdyz se dostaneme na jeho konec
...
*/
	...
	return q->f_index == q->b_index; //vraci 1 kdyz je prazda, jinak 0
}
...
*/
	...
	return nextIndex(q->b_index) == q->f_index; //vraci 1 jen kdyz je fronta plna, jinak 0
}
...
*/
	...
	if(queueEmpty(q)) queueError(QERR_FRONT);
	else *c = q->arr[q->f_index]; //front neni nutne 0, za dobu pouzivani se to muze menit a proto je pole zacyklene
}
...
*/
	...
	else q->f_index = nextIndex(q->f_index); //realne znak v poli zustane, jen presuneme ukazatel
}
...
*/
	...
	if(queueEmpty(q)) queueError(QERR_GET);
	else
	{
		queueFront(q, c); //pouze kombinace predeslych funkci
		queueRemove(q);
	}
}
...
*/
	...
	if(queueFull(q)) queueError(QERR_UP); //osetreni pro plnou frontu, jinak by se nam prepisovaly ulozene prvky
	else
	{
		q->arr[q->b_index] = c;
