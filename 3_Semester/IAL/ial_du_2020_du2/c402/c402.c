...
**/
	if(RootPtr == NULL) return; //neplatny ukazatel na stromu
...
**/
	...
	tBTNodePtr where = tmp;
	while(tmp != NULL && found == false) //vyhledani mista na vlozeni prvku
	{
		where = tmp;
		if(tmp->Cont > Content) tmp = tmp->LPtr;
		else if(tmp->Cont < Content) tmp = tmp->RPtr;
		else found = true;
	}

	if(found) //kdyz byl nalezen
	{
		return; //nic se nedeje
	}
	else //jinak
	{
		tBTNodePtr newIn = malloc(sizeof(struct tBTNode)); //alokujeme a inicializujeme novy uzel
		newIn->Cont = Content;
		newIn->LPtr = NULL;
		newIn->RPtr = NULL;

		if(where == NULL) //vkladame koren
		{
			(*RootPtr) = newIn;
		}
		else //vkladame list
		{
			if(where->Cont > Content) where->LPtr = newIn;
			else where->RPtr = newIn;
		}
...
**/
	...
	while(tmp != NULL) //dokud nejsem na prazdnem (neexistujicim) uzlu
	{
		BTWorkOut(tmp);
		SPushP(Stack, tmp);
		tmp = tmp->LPtr;
...
**/
	tBTNodePtr tmp = NULL;
	tStackP stack; //vytvorim zasobnik a inicializuju ho
	SInitP(&stack);
	Leftmost_Preorder(RootPtr, &stack);
	while(!SEmptyP(&stack))
	{
		tmp = STopPopP(&stack);
		Leftmost_Preorder(tmp->RPtr, &stack); //funkci workout vola predesla funkce
	}
...
**/
	...
	while(tmp != NULL)
	{
		SPushP(Stack, tmp); //analogie Preorder, ale nevolam zde workout
		tmp = tmp->LPtr;
	}
...
**/
	...
	{
		tmp = STopPopP(&stack);
		BTWorkOut(tmp); //analogie Preorder, ale workout volam zde
...
**/
	...
	{
		SPushP(StackP, tmp);
		SPushB(StackB, true); //ukladame informaci ze byl navstiven poprve
		tmp = tmp->LPtr;
	}
...
**/
	...
	tBTNodePtr tmp = RootPtr;
	tStackP stackP;
	tStackB stackB; //vytvorime take zasobnik pro bool hodnoty

	SInitP(&stackP);
	SInitB(&stackB); //inicializujeme oba zasobniky

	Leftmost_Postorder(tmp, &stackP, &stackB); //jednou zavolame funkci
	while(!SEmptyP(&stackP)) //staci kontrolovat stack na zasobnik, pushuje se na ne zaroven
	{
		tmp = STopPopP(&stackP);
		zleva = STopPopB(&stackB);

		if(zleva) //pokud je projity zleva, projde se jeste zprava
		{
			SPushP(&stackP, tmp);
			SPushB(&stackB, false);
			Leftmost_Postorder(tmp->RPtr, &stackP, &stackB);
		}
		else BTWorkOut(tmp); //jinak se zpracuje
...
**/
	...
	if(RootPtr == NULL) return; //strom neexistuje

	tBTNodePtr tmp = (*RootPtr);
	tStackP stack;
	SInitP(&stack);
	Leftmost_Inorder(tmp, &stack); //pouzijeme na naplneni zasobniku Inorder funkci (ta jedina nevola workout a neuklada bool hodnoty)

	while(!SEmptyP(&stack)) //dokud neni zasobnik prazdny tak prochazime prave potomky a volame na ne Leftmost_Inorder, uvolnujeme dany uzel
	{
		tmp = STopPopP(&stack);
		Leftmost_Inorder(tmp->RPtr, &stack);
		tmp->LPtr = NULL;
	...
	(*RootPtr) = NULL; //vraceni do stavu po inicializaci
